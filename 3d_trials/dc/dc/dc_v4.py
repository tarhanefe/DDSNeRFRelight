...
class DC(object):
    ...
    ############################################################################
    def wave_grad(self, grad: torch.Tensor, wavelet: str = 'db1', level: int = 1) -> torch.Tensor:

        """
        Extracts the low-pass (approximation) component of the gradient using wavelet decomposition.
        ...
        Returns:
            Low-pass filtered tensor of same shape as input [B, C, H, W]
        """
        B, C, H, W = grad.shape
        grad_lp = torch.zeros_like(grad)

        for b in range(B):
            for c in range(C):
                x_np = grad[b, c].detach().cpu().numpy()

                # Multi-level wavelet decomposition
                coeffs = pywt.wavedec2(x_np, wavelet=wavelet, level=level)

                # Keep only the approximation coefficients (lowest resolution)
                cA = coeffs[0]
                coeffs_lp = [cA] + [(None, None, None)] * level

                # Reconstruct from low-pass only
                rec_lp = pywt.waverec2(coeffs_lp, wavelet=wavelet)

                # Crop to original shape if needed
                rec_lp = rec_lp[:H, :W]

                grad_lp[b, c] = torch.tensor(rec_lp, device=grad.device)
